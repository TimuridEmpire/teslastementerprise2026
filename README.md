...
Important website env values — **all server-side only, never `NEXT_PUBLIC_*`**.
The browser never holds a router credential: every client component calls
this site's own `/api/router/*` proxy (`app/api/router/[...path]/route.ts`),
which resolves the real credential from these on the server and attaches
it. (Earlier versions of this project read these via `NEXT_PUBLIC_*`, which
Next.js inlines into the client JS bundle at build time — that shipped the
admin secret and every agent's API key to the browser in plaintext. Fixed;
see the Enterprise Deployment Blueprint's critical-path findings.)

- `ROUTER_URL` — where the router actually is (`http://localhost:8000` locally, `http://router:8000` in the Docker Compose bundle)
...
The browser never holds a router credential. Every client component calls
this site's own server-side proxy (`app/api/router/[...path]/route.ts`),
which resolves the real credential from its own env and attaches it —
`ROUTER_URL`, `ENTERPRISE_ROUTER_ADMIN_SECRET`, and one `*_AGENT_API_KEY`
per department, listed in full a few sections up ("Important website env
values"). None of these are `NEXT_PUBLIC_*` — that would inline them into
the client JS bundle at build time, which is exactly how this project used
to ship its admin secret and every agent's API key to the browser in
plaintext.
