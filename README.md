...
### 8. Verify Live Agent Output

Use the router and website together. Queues can look empty if workers fetch and ack quickly; completed work should appear in audit history and artifacts.
...
- /dashboard: latest markdown artifact from a live agent run.
- /observability: audit count, throughput, and router lifecycle events.
- /agents/ceo: CEO strategy output after Ollama succeeds.
- /agents/product, /agents/hr, /agents/engineering: routed outputs after those workers process messages.
- /messages: the live manager queue, not a full historical message archive.

### Seeing Agent Responses

Agent responses are not chat bubbles yet. The current proof of work is:

- Router messages entering queues.
- Workers fetching leased messages.
- Ack/nack lifecycle in audit history.
- Markdown artifacts shown through the router artifact API and website panels.

If the queue page is empty but audit/artifacts changed, the workflow probably succeeded and the worker already acked the message.
...
- Ollama is not installed, or the current terminal has not picked up the updated PATH.
...
### `Strategic Link Error` or `localhost:11434` connection refused

Likely cause:

- CEO tried to call local Ollama, but Ollama is not running or the model is not available.
...
If the error says the `mistral` model is missing, run `ollama pull mistral`.
